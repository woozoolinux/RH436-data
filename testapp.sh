...
testapp_start() {
        ...
        if [ "x${start_path}" = "x" ]; then
                ocf_log info "testapp_start: start-script is NOT defined. OCF_SUCCSS($OCF_SUCCESS) is returned."
                return $OCF_SUCCESS
        fi

        if [ ! -f ${start_path} ]; then
                ocf_log err "testapp_start: $start_path is NOT a regular file."
                return $OCF_ERR_CONFIGURED
        fi

        if [ ! -s ${start_path} ]; then
                ocf_log err "testapp_start: $start_path is a empty file."
                return $OCF_ERR_CONFIGURED
        fi

        if [ ! -x ${start_path} ]; then
                ocf_log err "testapp_start: $start_path can't be executable."
                return $OCF_ERR_PERM
        fi
...
testapp_stop() {
        ...
        if [ "x${stop_path}" = "x" ]; then
                ocf_log info "testapp_stop: stop-script is NOT defined. OCF_SUCCSS($OCF_SUCCESS) is returned."
                return $OCF_SUCCESS
        fi

        if [ ! -f ${stop_path} ]; then
                ocf_log err "testapp_stop: $stop_path is NOT a regular file."
                return $OCF_ERR_CONFIGURED
        fi

        if [ ! -s ${stop_path} ]; then
                ocf_log err "testapp_stop: $stop_path is a empty file."
                return $OCF_ERR_CONFIGURED
        fi

        if [ ! -x ${stop_path} ]; then
                ocf_log err "testapp_stop: $stop_path can't be executable."
                return $OCF_ERR_PERM
        fi
...
testapp_monitor() {
        ...
        if [ "x${monitor_path}" = "x" ]; then
                ocf_log info "testapp_monitor: monitor-script is NOT defined. OCF_SUCCSS($OCF_SUCCESS) is returned."
                return $OCF_SUCCESS
        fi

        if [ ! -f ${monitor_path} ]; then
                ocf_log err "testapp_monitor: $monitor_path is NOT a regular file."
                return $OCF_ERR_CONFIGURED
        fi

        if [ ! -s ${monitor_path} ]; then
                ocf_log err "testapp_monitor: $monitor_path is a empty file."
                return $OCF_ERR_CONFIGURED
        fi

        if [ ! -x ${monitor_path} ]; then
                ocf_log err "testapp_monitor: $monitor_path can't be executable."
...
stop)           testapp_stop
                ;;
monitor|status) testapp_monitor
                ;;
validate-all)   testapp_validate
                ;;
usage|help)     testapp_usage
                exit $OCF_SUCCESS # Must exit with $OCF_SUCCESS.
                ;;
*)              testapp_usage
